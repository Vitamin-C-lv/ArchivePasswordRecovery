...
try {
    ...
    $futureAppend = [pscustomobject]@{
        CoverageId = 'builtin:l3-append-global'; Kind = 'RuleAppendVariants'; DisplayName = 'L3 suffix variants'; DictionarySource = 'Builtin'
        CandidateCount = 600L; EngineStrategy = 'Dictionary'; GpuSupported = $true; StageNumber = 3
    }
    $items = @($current, $futureCase, $futureAppend)
    $casePath = Join-Path (Join-Path $script:RuntimeDirectory 'dictionaries') 'rule-case-builtin_l2-case-global.txt'
    $appendPath = Join-Path (Join-Path $script:RuntimeDirectory 'dictionaries') 'rule-append-builtin_l3-append-global.txt'
    ...
    Assert-True (-not (Test-Path -LiteralPath $appendPath -PathType Leaf)) 'Fresh selection materialized the L3 append source.'
    ...
    Assert-True ($cachedIds -notcontains 'builtin:l3-append-global') 'The unready L3 append coverage was skipped instead of stopping expansion.'
